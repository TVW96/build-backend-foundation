-- MangaMarketplace application schema reset.
-- Destructive: all data in the listed public tables is removed.
-- Supabase-managed auth, storage, and system tables are not dropped.

begin;

drop table if exists public.listing_item_images cascade;
drop table if exists public.inventory_item_images cascade;
...
create index idx_listing_items_item_id
  on public.listing_items(item_id);
...
create index idx_entity_images_asset_id
...
-- Deny direct Data API access. The Nest API connects with a server-side
-- database role and remains the authorization boundary.
...
-- Public reads are CDN-backed. Public bucket status does not grant upload,
